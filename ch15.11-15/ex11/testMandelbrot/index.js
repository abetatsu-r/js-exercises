...
class WorkerPool {
  constructor(numWorkers, workerSource) {
    this.idleWorkers = [];
    this.workQueue = [];
    this.workerMap = new Map(); // warkerと解決、失敗時の関数のMap
...
class MandelbrotCanvas {
  ...
  // constructorおよびresize時に実行される
  setSize() {
    this.width = this.canvas.width = window.innerWidth;
    this.height = this.canvas.height = window.innerHeight;
    this.tiles = [...Tile.tiles(this.width, this.height, ROWS, COLS)]; // タイルは3*4
  }
  ...
  setState(f, save = true) {
    ...
    if (save) {
      history.pushState(this.state, "", this.state.toURL()); // stateはcx, cyみたいなの
    }
  }
    ...
    this.pendingRender = Promise.all(promises)
      .then((responses) => {
        ...
        for (let r of responses) {
          ...
          if (r.max > max) max = r.max; // 途中で打ち切るので打ち切られた範囲を記載する
        }
